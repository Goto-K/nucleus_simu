...
CCC   208Pb
      ...
      at=rhoitg(rmin,rmax,dr,Rp,rho0,ad) ![MeV]
...
C
C=======================================================================
      FUNCTION RHOITG(RMIN,RMAX,DR,Rp,rho0,ad)
...
C
      ...
      DO IR=1,NRMAX
       ...
       IF(Q.LT.1.0D-10) THEN
        ...
        ELSE
         FITG=FITG+(rho0/(1+exp((r-Rp)*ad)))
       END IF
      END DO
C
      IF(Q.LT.1.0D-10) THEN
        FITG=FITG-(rho0/(1+exp((rmin-Rp)*ad)))*RMIN*0.5D0
     1           -(rho0/(1+exp((rmax-Rp)*ad)))*RMAX*0.5D0
       ELSE
        FITG=FITG-(rho0/(1+exp((rmin-Rp)*ad)))*0.5D0
     1           -(rho0/(1+exp((rmax-Rp)*ad)))*0.5D0
      END IF
C
      FITG=FITG*DR
C
