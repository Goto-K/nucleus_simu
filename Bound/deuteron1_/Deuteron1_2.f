...
CCC   kinetic energy matrix
      ...
      if(l1==1.and.l2==1)then
      T(i,j,l1,l2)
     & = CN0(i)*CN0(j)*hc**2/rem 
     & * sqrt(pi)/sqrt(b(i)+b(j))/4/(b(i)+b(j))
     & * 6*b(i)*b(j)/(b(i)+b(j))
      else if(l1==3.and.l2==3)then
      T(i,j,l1,l2)
     & = CN2(i)*CN2(j)*hc**2/rem 
...
CCC   |f> = Σ_a evec/√eig |fai>
      ...
      do i=1,nmax
       do j=1,nmax
        vecs(i,j)=0
        do k=1,nmax
         vecs(i,j) = vecs(i,j) + evec1(i,k) * evec2(j,k)
        end do
       end do
      end do
...
CCC   bound state
      ...
      wf2=0d0
      do i=1,nmax
       wf0=wf0+vec7(i,k)*CN0(i)*r*exp(-b(i)*r**2)
       wf2=wf2+vec8(i,k)*CN2(i)*r**3*exp(-b(i)*r**2)
      end do
C      write(*,*) r,wf0,wf2
      end do
