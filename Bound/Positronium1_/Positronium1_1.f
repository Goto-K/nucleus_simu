...
CCC   spin-orbit coupling
CCC  
CCC   VSO=VLS(r)*h^2/2*(j(j+1)-l(l+1)-3/4)
CCC   j=|l-1/2|,l+1/2      
CCC   VLS(r)=-(g-1)(eh^2/(2m^2c^2))(1/r*dVC/dr)
CCC   E{→}(r)=-grad(V(r))=-r{→}(1/r*d(VC)/dr)
CCC   ge=2.002319と仮定
CCC
     ...
     !VSO=VLS*...*(3/4-0-3/4)=0
      ...
      j1=ls-1d0/2d0
      j2=ls+1d0/2d0
      so1=hc**2/(2*ec)*(j1(j1+1d0)-ls(ls+1d0)-3d0/4d0)
      so2=hc**2/(2*ec)*(j2(j2+1d0)-ls(ls+1d0)-3d0/4d0)
      end if
      VSO1(i,j)=VLS(i,j)*so1
      VSO2(i,j)=VLS(i,j)*so2
