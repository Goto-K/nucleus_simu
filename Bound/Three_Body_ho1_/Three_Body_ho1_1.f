...
CCC   *
     ...
     &           nmax=3,mmax=10,omax=3,pmax=10)
      real*8,dimension(ndim,ndim)  :: 
     &          N1,N2,T,V,H,t1,hvec,evec1,evec2,
     &          cn1,cn2
      complex*16,dimension(ndim,ndim)  :: 
...
CCC   standardization
      ...
      do i=1,nmax*omax
       do j=1,nmax*omax
       if(eig1(j).lt.0) then
        vec1(i,j) = evec1(i,j) / sqrt(abs(eig1(j))) * ci
       else
...
C     &              + vec1(k,i)*cmplx(H(k,l),0,kind(0d0))*vec1(l,j)
         end do
        end do
       end do
      end do
      ...
      do i=1,nmax*omax
      write(*,'(F10.5,9(1X,F10.5))') (hvec(i,j),j=1,nmax*omax)
      end do
      do i=1,nmax*omax
      write(*,'(F10.5,9(1X,F10.5))') (tvec1(i,j),j=1,nmax*omax)
