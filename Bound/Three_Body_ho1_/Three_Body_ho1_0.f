...
CCC
     ...
     &         * sqrt((((2d0*(bx(i)/4d0+9d0/16d0*By(k)))**3)/pi)))
     &         * sqrt(2d0**2/1d0
     &         * sqrt((((2d0*(bx(i)+By(k)/4d0))**3)/pi)))
...
C    <1|1>+<1|2>+<1|3>
     ...
     & = cn1(i,k)*cn1(j,l)/2d0**2 * sqrt((pi)/((bx(i)+bx(j))**3)) 
     ...
     & * 1d0/2d0**2 * sqrt((pi)/((bx(j)+By(k)+By(l)/4d0)**3))
      ...
      N1(omax*(j-1)+l,omax*(i-1)+k)=N1(omax*(i-1)+k,omax*(j-1)+l)
      N2(omax*(i-1)+k,omax*(j-1)+l)=N1(omax*(i-1)+k,omax*(j-1)+l)
      N2(omax*(j-1)+l,omax*(i-1)+k)=N2(omax*(i-1)+k,omax*(j-1)+l)
...
C    <1|T(r1)|1>+<1|T(r1)|2>+<1|T(r1)|3>
     ...
     & = cn1(i,k)*cn1(j,l)*hc**2 / (2d0*rem1)
     & * 3d0/2d0*bx(i)*bx(j) * sqrt((pi)/(bx(i)+bx(j))**5)
     & * 1d0/2d0**2 * sqrt((pi)/((By(k)+By(l))**3))
     & + 2d0 
     & * cn1(i,k)*cn2(j,l)*hc**2 / (2d0*rem1)
...
CCC   <1|V1|1> + 2*<1|V1|2> + 2*<1|V2|1> + 4*<1|V2|3>
     ...
     & = cn1(i,k)*cn1(j,l)*rem1 / (2*hc**2)
     & * 3d0/8d0 * sqrt((pi)/(bx(i)+bx(j))**5)
     & * 1d0/2d0**2 * sqrt((pi)/((By(k)+By(l))**3))

     & + 2d0
     & * cn1(i,k)*cn2(j,l)*rem1 / (2*hc**2)
     ...
     & * 3d0/8d0 * sqrt((pi)/(bx(j)+By(k)+By(l)/4d0)**5)
      ...
      V(omax*(j-1)+l,omax*(i-1)+k)=V(omax*(i-1)+k,omax*(j-1)+l)
...
CCC   hamiltonian matrix
     ...
     & = T(omax*(i-1)+k,omax*(j-1)+l)+V(omax*(i-1)+k,omax*(j-1)+l)
      ...
      H(omax*(j-1)+l,omax*(i-1)+k)=H(omax*(i-1)+k,omax*(j-1)+l)
...
CCC
      ...
      call hqr(ndim,nmax*omax,N1,evec1,eig1)
...
C         vec2(i,j) = vec2(i,j) + evec1(k,i)*N2(k,l)*evec1(l,j)
     &                           /sqrt(eig1(i)*eig1(j))
         end do
        end do
       end do
      ...
      do i=1,nmax*omax
C      write(*,'(F10.5,9(1X,F10.5))') (vec2(i,j),j=1,nmax*omax)
      write(*,'(F10.5,9(1X,F10.5))') (n2(i,j),j=1,nmax*omax)
C      write(*,*) eig1(i)
      end do
...
CCC
      ...
      call hqr(ndim,nmax*omax,hvec,evec2,ene1)
...
C      do k=1,10
C      do i=1,nmax
C       wf0(k)=wf0(k)+vec4(i,k)*CN0(i)*exp(-b(i)*r**2)
C       wf2(k)=wf2(k)+vec4(nmax+i,k)*CN2(i)*r**2*exp(-b(i)*r**2)
C      end do
C      end do

C      write(*,*) r,wf0(1),wf2(1)
C      end do
