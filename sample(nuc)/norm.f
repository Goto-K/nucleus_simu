...
CCC   ---------------H(I,J)-----------------------
     ...
     &     -HC/EC*CN(I)*CN(J)*SF0/(2D0                !   +<fai_i|-e^2/r|fai_j>
     &     *(ALP(I)+ALP(J))**(LS+1))
      H(J,I)=H(I,J)
CCC   ----------------END DO----------------------
 120  CONTINUE
...
c      WRITE(*,*)
c      WRITE(*,*) I,ENE3(I),-HC/EC/2D0/A_0/(LS+I)**2
      END DO
      ...
      WF=0D0
      DO I=1,NMAX
      WF=WF+VEC7(I,K)*CN(I)*R**(LS+1)*DEXP(-ALP(I)*R**2)
      END DO
      WRITE(*,*) R,WF,2D0*R*DEXP(-R/A_0)/DSQRT(A_0)**3
      END DO
